      ...
      savePathButton.addEventListener('click', () => {
            ...
            if (routeName) {
                  ...
                  if (template) {
                        ...
                        fetch("http://localhost:3000/api/ruta-crear?name=" + routeName)
                              .then(response => response.text()) // Asegúrate de llamar al método text()
                              .then(value => alert(value))
                              .catch(error => console.error('Error:', error)); // Captura errores en el fetch
